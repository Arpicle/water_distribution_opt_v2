...
def run_one_case(env: WaterAllocationEnv, model, device: torch.device, seed: int, deterministic: bool) -> dict:
...
def main(config: TestConfig = CONFIG) -> None:
    ...
    cases = []
    for case_idx in range(config.num_demands):
        seed = config.seed + case_idx
        print(f"test_case={case_idx + 1:04d}/{config.num_demands} seed={seed}")
        cases.append(run_one_case(env, model, device, seed, config.deterministic))
